...
from utils import log_weights_to_weights
...
class ExternalFieldIsingModel(SMCSampler):

    def __init__(self, dimension, alpha, magnetic=0.0):
        """ sequential monte carlo sampler for the mean field Ising model. Uses tempering to move from the uniform
        distribution to the temperature of interest (alpha). Markov transitions accomplished via Gibb's sampling.

        The log density of the target distribution is given by:

        (alpha*M**2 + magnetic*M)/dimension

        Where M is the sum of the spins, i.e. the total magnetization.
        ...
        alpha: float > 0
            the temperature of the model, determining the behaviour
        magnetic: float
            biases the model towards positive or negative spins.
        """

        self.dimension = dimension
        self.alpha = np.float(alpha)
        self.magnetic = np.float(magnetic)
    ...
    def _log_pdf(self, samples, params):
        ...
        # compute the pdf
        M = samples.sum(1)
        log_density = beta * (self.alpha * M**2 + self.magnetic * M) / self.dimension
    ...
    def _markov_kernel(self, samples, params, kernel_steps=10):
        ...
        for k in range(kernel_steps):
            for d in range(self.dimension):
                # compute log probability of state 1
                M1 = magnetism - samples[:, d] + 1.0
                energy1 = beta*(self.alpha * M1**2 + self.magnetic * M1)/self.dimension

                # compute the log probability of state 0
                M0 = magnetism - samples[:, d] - 1.0
                energy0 = beta * (self.alpha * M0 ** 2 + self.magnetic * M0) / self.dimension

                probabilities = expit(energy1 - energy0)  # probability of transitioning to +1
    ...
    def _true_probabilities(self, params):
        ...
        for d in range(self.dimension + 1):
            magnetism[d] = -self.dimension + 2 * d
            probabilities[d] = comb(self.dimension, d) * \
                np.exp(beta * (self.alpha * magnetism[d] ** 2 + self.magnetic * magnetism[d]) / self.dimension)
    ...
    def _total_variation(self, samples, log_weights, params=(1.0,)):
        """ returns the total variation distance between the empirical distribution and the true distribution
        for a specified inverse temperature
        ...
        log_weights: np.array
            (N, ) array of log weights from sampling
        params: tuple
            first value is the inverse temperature parameter in [0,1]
        ...
        true_probabilities, true_magnetism = self._true_probabilities(params)

        sample_magnetism = samples.sum(1)
        empirical_probabilities = np.array([weights[sample_magnetism == m].sum() for m in true_magnetism])
    ...
    def plot_diagnostics(self, output, path, max_steps=50):
        ...
        plt.subplot(223)
        tvs = []
        for sample, log_weight, params in zip(samples, log_weights, path):
            tvs.append(self._total_variation(sample, log_weight, params))
        plt.plot(tvs)
        plt.title('Total variation distance')
        plt.xlabel('iteration')
        plt.ylim(-0.05, 1.05)
